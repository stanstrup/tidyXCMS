...
- MsFeatures grouping (retention time, abundance correlation, EIC similarity) - **optional**
- Sample metadata
...
**Note:** Both CAMERA annotations and MsFeatures grouping are optional. You can create
a peak table with either, both, or neither of these annotation types.
...
## MsFeatures Grouping (Optional)

In addition to CAMERA annotations, you can use the `MsFeatures` package to group features based on retention time similarity, abundance correlation, or EIC similarity. This provides an alternative or complementary approach to CAMERA's pseudospectrum grouping.
...
### Combining CAMERA and MsFeatures

You can use both CAMERA annotations and MsFeatures grouping together. Apply `groupFeatures()` first, then proceed with CAMERA annotation:
...
xset_grouped <- xdata_grouped %>%
  as("xcmsSet")

# Apply CAMERA annotation as before
xs_grouped <- xsAnnotate(xset_grouped, polarity = "positive")
xs_grouped <- groupFWHM(xs_grouped, perfwhm = 0.1, intval = "into", sigma = 6)
xs_grouped <- groupCorr(xs_grouped, calcIso = FALSE, calcCiS = TRUE, calcCaS = FALSE,
                        cor_eic_th = 0.7, pval = 1E-6)
xs_grouped <- findIsotopes(xs_grouped, ppm = 10, mzabs = 0.01, intval = "into", maxcharge = 2)

# Get adduct rules (reuse from earlier)
rules_pos <- MZ_CAMERA(mode = "pos", warn_clash = TRUE, clash_ppm = 5) %>%
  filter(name != "[M+NH4]+") %>%
  as.data.frame()

xs_grouped <- findAdducts(xs_grouped, ppm = 500, mzabs = 0.2, multiplier = 4,
                          polarity = "positive", rules = rules_pos)

# Create peak table with both CAMERA and MsFeatures annotations
peak_table_combined <- XCMSnExp_CAMERA_peaklist_long(xdata_grouped, xs_grouped)

# Check that both types of annotations are present
c("feature_group", "isotopes", "adduct", "pcgroup") %in% colnames(peak_table_combined)
