...
```{r camera_rules, message=FALSE}
# Get adduct/fragment rules for CAMERA
...
The `commonMZ::MZ_CAMERA()` function provides a curated list of adducts and neutral losses commonly observed in LC-MS data. Because an NH4+ adduct and a neutral loss of NH3 give the same mass difference, you may see warnings about clashing annotations. In such cases, you can filter out less common adducts:

```{r camera_rules_filter, message=FALSE, eval=FALSE}
# Example: Remove [M+NH4]+ if it's less common in your data
rules_pos <- MZ_CAMERA(mode = "pos", warn_clash = TRUE, clash_ppm = 5) %>%
  filter(name != "[M+NH4]+") %>%
  as.data.frame()
```
...
xs <- findAdducts(xs, ppm = 10, mzabs = 0.01, multiplier = 4, polarity = "positive", rules = rules_pos)
```
