...
`tidy_peaklist()` function to create peak tables from XCMS results.
...
- **`feature_group`**: Groups features based on retention time and correlation across samples (from MsFeatures)
- **`pcgroup`**: Groups features into pseudospectra based on EIC similarity (from CAMERA)
...
```{r missing_values, fig.alt="Histogram showing the distribution of feature detection rates across samples. The x-axis shows detection rate from 0 to 1, and the y-axis shows the frequency (count) of features at each detection rate."}
