# Understanding Version Numbers in tidyXCMS

## The Confusion: Why Do Commit Messages Show v2.0.2?

If you look at the git history, you’ll see confusing commit messages
like:
...
This project uses a special two-stage versioning to meet Bioconductor
requirements:

### Stage 1: Semantic-Release Calculates Version

- Semantic-release analyzes commit messages (`feat:`, `fix:`, etc.)
- It calculates what the version **would be** using standard semantic
  versioning
...
**Bioconductor Requirement:** - Bioconductor requires new packages to
start with version \< 1.0.0 - Specifically, they expect versions in the
0.99.x range - After acceptance, the package can move to 1.0.0+

**Semantic-Release Benefit:** - We still want automatic versioning based
on commit messages - Semantic-release is the industry standard tool for
this - Rather than fight it, we let it calculate versions then convert
them

## What Actually Matters

When working with this repository, **ignore the commit message
versions**. Always look at:
...
``` bash
$ git log --oneline -1 64996f9
64996f9 chore(release): 2.0.2 [skip ci]

$ git show 64996f9:DESCRIPTION | grep Version
Version: 0.99.17

$ git tag --points-at 64996f9
0.99.17
```

The commit message says “2.0.2” but the actual version is “0.99.17”.
...
- `.github/prepare-news.sh` - Converts semantic-release versions to
  0.99.x
...
|-----------------|--------------------------------|-------------------|---------------------|
| Commit messages | Semantic versioning            | 2.0.2             | Ignore this         |
| DESCRIPTION     | Bioconductor format            | 0.99.17           | **Source of truth** |
| NEWS.md         | Bioconductor format            | 0.99.17           | Matches DESCRIPTION |
| Git tags        | Bioconductor format            | 0.99.17           | Matches DESCRIPTION |
| GitHub Releases | ~~Semantic~~ Should match tags | ~~2.0.2~~ 0.99.17 | Need to fix         |
